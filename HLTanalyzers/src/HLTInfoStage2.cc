...
#include "HLTrigger/HLTanalyzers/interface/HLTInfoStage2.h"
...
void HLTInfoStage2::beginRun(const edm::Run& run, const edm::EventSetup& c){ 
...
void HLTInfoStage2::setup(const edm::ParameterSet& pSet, TTree* HltTree) {
...
/* **Analyze the event** */
void HLTInfoStage2::analyze(const edm::Handle<edm::TriggerResults>                 & hltresults,
...
//   std::cout << " Beginning HLTInfoStage2 " << std::endl;


  /////////// Analyzing HLT Trigger Results (TriggerResults) //////////
  if (hltresults.isValid()) {
    int ntrigs = hltresults->size();
    if (ntrigs==0){std::cout << "%HLTInfoStage2 -- No trigger name given in TriggerResults of the input " << std::endl;}
    ...
    for (int itrig = 0; itrig != ntrigs; ++itrig){
      ...
      if (_Debug){
        if (_Debug) std::cout << "%HLTInfoStage2 --  Number of HLT Triggers: " << ntrigs << std::endl;
        std::cout << "%HLTInfoStage2 --  HLTTrigger(" << itrig << "): " << trigName << " = " << accept << std::endl;
      }
    }
  }
  else { if (_Debug) std::cout << "%HLTInfoStage2 -- No Trigger Result" << std::endl;}
  ...
  if (L1ExtEmIsol.isValid()) {
    ...
    nl1extiem = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No Isolated L1 EM object" << std::endl;
  }
  ...
  if (L1ExtEmNIsol.isValid()) {
    ...
    nl1extnem = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No Non-Isolated L1 EM object" << std::endl;
  }
  ...
  if (L1ExtMu.isValid()) {
    ...
    nl1extmu = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 MU object" << std::endl;
  }
  ...
  if (L1ExtJetC.isValid()) {
    ...
    nl1extjetc = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 Central JET object" << std::endl;
  }
  ...
  if (L1ExtJetF.isValid()) {
    ...
    nl1extjetf = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 Forward JET object" << std::endl;
  }
  ...
  if (L1ExtJet.isValid()) {
    if (_Debug) std::cout << "%HLTInfoStage2 -- Found L1 JET object" << std::endl;
    nl1extjet = maxL1Jet;
    ...
    //    nl1extjetf = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 JET object" << std::endl;
  }
  ...
  if (L1ExtTau.isValid()) {
    ...
    nl1exttau = 0;
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 TAU object" << std::endl;
  }
  ...
  if (L1ExtMet.isValid()) {
    ...
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 MET object" << std::endl;
  }
  ...
  if (L1ExtMht.isValid()) {
    ...
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 MHT object" << std::endl;
  }
  ...
  if (L1GTRR.isValid()) {  
    ...
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 GT ReadoutRecord " << std::endl;
  }
  ...
  if (gctBitCounts.isValid()) {
    ...
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 Gct HF BitCounts" << std::endl;
  }
  ...
  if (gctRingSums.isValid()) {
    ...
    if (_Debug) std::cout << "%HLTInfoStage2 -- No L1 Gct HF RingSums" << std::endl;
  }

  if (_Debug) std::cout << "%HLTInfoStage2 -- Done with routine" << std::endl;
}
